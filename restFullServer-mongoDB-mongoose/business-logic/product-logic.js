...
function getAllProductAsync() {
  return Product.find().exec(); //exec()-> function return promis
}
...
async function upddateProductAsync(product) {
  const info = await Product.updateOne({ _id: product._id }, product).exec(); //updateOne() A-exec() lo mahzira object product ela object shel meyda(info) lahen afunction hi async
...
function getAllProductByRangeAsync(minPrice, maxPrice) {
  ...
  return Product.find({ price: { $gte: minPrice, $lte: maxPrice } }, null, {
    sort: { price: -1, name: 1 },
  }).exec();
