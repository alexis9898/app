...
router.post("/", async (request, response) => {
  try {
    ...
    const addCustomer = await CustomerLogic.addCustomerAsync(customer,cart);
    console.log("++++++++++++++++++00");
...
router.post("/add-product-to-cart/:_id", async (request, response) => { //_id of customer
  try {
    ...
    const addProductCart = request.body; //customerId , productId, sum
       
    customer.cartProductId.push(addProductCart);

    const addProduct = await CustomerLogic.addCustomerAsync(customer);
    console.log("++++++++++++++++++00");
